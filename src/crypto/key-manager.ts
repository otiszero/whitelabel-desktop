 ...
 */
function deriveAddress(chain: ChainType, publicKey: Buffer, privateKey: Buffer): string {
  switch (chain) {
    case 'btc':
 ...
 */
function deriveBtcAddress(publicKey: Buffer): string {
 ...
 */
function deriveXrpAddress(publicKey: Buffer): string {
  const sha = sha256(publicKey);
  const hash160 = ripemd160(sha);
 ...
 */
function deriveTronAddress(publicKey: Buffer): string {
  const uncompressed = decompressPublicKey(publicKey);
  const hash = keccak_256(uncompressed.slice(1));
 ...
 */
function decompressPublicKey(compressed: Buffer): Buffer {
  if (compressed.length === 65) return compressed;
 ...
 */
function bech32Encode(hrp: string, version: number, data: Buffer): string {
  ...
  function convertBits(data: Buffer, fromBits: number, toBits: number, pad: boolean): number[] {
    let acc = 0, bits = 0;
    const ret: number[] = [], maxv = (1 << toBits) - 1;
  ...
  for (let i = 0; i < 6; i++) checksumChars.push((checksum >> (5 * (5 - i))) & 31);
 ...
 */
function base58CheckEncode(payload: Buffer, version: number | null, alphabet: 'bitcoin' | 'ripple'): string {
  const ALPHABETS = {
