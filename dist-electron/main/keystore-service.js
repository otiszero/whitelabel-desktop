...
import { createKeystore, decryptKeystore, updateKeystoreWallets, verifyPassword, validatePassword, } from '../crypto/keystore.js';
import { generateMnemonic, validateMnemonic, mnemonicToSeed, deriveKeyPair, } from '../crypto/key-manager.js';
 ...
 */
export async function createNewKeystore(password, mnemonic) {
    try {
        // Validate password
 ...
 */
export async function unlockKeystore(password) {
    try {
        if (!keystoreExists()) {
 ...
 */
export async function createWallet(chain, name, password) {
    if (!unlockedKeystore) {
    ...
    try {
        ...
        const newKeystore = await updateKeystoreWallets(unlockedKeystore.keystoreFile, password, updatedWallets);
 ...
 */
export function getPrivateKey(walletId) {
    if (!unlockedKeystore) {
        return null;
    }
 ...
 */
export function getAddress(walletId) {
    if (!unlockedKeystore) {
        return null;
    }
    const wallet = unlockedKeystore.keystoreFile.wallets.find(w => w.id === walletId);
